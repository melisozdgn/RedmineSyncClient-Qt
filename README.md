...
* **Offline Access (Local Persistence):** I used `QJsonDocument` to create a local storage system. When the app starts, it loads everything from a local `tasks.json` file first, so the interface is ready immediately without waiting for a server response.
